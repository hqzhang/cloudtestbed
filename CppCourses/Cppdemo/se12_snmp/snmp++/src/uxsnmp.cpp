...
char snmp_cpp_version[]="#(@) SNMP++ $Id: uxsnmp.cpp,v 1.21 2005/09/19 19:59:36 fock Exp $";
			      ...
			      const UdpAddress &addr)
{
  ...
  do
  {
    ...
    if ((nfound > 0) && (FD_ISSET(socket, &readfds)))
    {
      // receive message
      UdpAddress from;
      int res = receive_snmp_response(socket, *this, dummy_pdu,
				      from, true /* process_msg */);
