/*_############################################################################
  ...
  _##  
  _##########################################################################*/
char mp_v3_cpp_version[]="@(#) SNMP++ $Id: mp_v3.cpp,v 1.18 2005/12/10 14:03:21 katz Exp $";
                     ...
                     UdpAddress from_address)
{
  ...
  // process scopedPDU
  debughexcprintf(21, "scoped PDU", scopedPDU, scopedPDULength);
	...
	*/
	if (usm->is_discovery_enabled())
	{
	  ...
	  if (reportableFlag)
	  {
	    ...
	    send_report(0, MAXLENGTH_BUFFER, pdu, SNMPv3_MP_INVALID_ENGINEID,
