/*_############################################################################
  ...
  _##  
  _##########################################################################*/
char usm_v3_cpp_version[]="@(#) SNMP++ $Id: usm_v3.cpp,v 1.20 2005/12/10 18:39:15 katz Exp $";
...
{
  Buffer<unsigned char> buffer(MAXLENGTH_BUFFER);
  Buffer<unsigned char> buffer2(MAXLENGTH_BUFFER);
  ...
  if (securityLevel == SNMP_SECURITY_LEVEL_AUTH_PRIV)
  {
    ...
    bufLength = bufPtr - buffer.get_ptr();
  ...
  totalLength += bufPtr - buffer.get_ptr();
