/*_############################################################################
  ...
  _##  
  _##  Stuttgart, Germany, Tue Dec 13 21:30:23 CET 2005 
  _##  
  _##########################################################################*/
/*===================================================================
...
=====================================================================*/
char notifyqueue_version[]="#(@) SNMP++ $Id: notifyqueue.cpp,v 1.12 2005/09/22 14:05:59 katz Exp $";
...
int CNotifyEvent::Callback(SnmpTarget &target, Pdu &pdu, int fd, int status)
{
  Oid trapid;
...
CNotifyEventQueue::CNotifyEventQueue(EventListHolder *holder, Snmp *session)
  : m_head(NULL,NULL,NULL), m_msgCount(0), m_notify_fds(0),
    m_notify_fd_count(0), my_holder(holder), m_snmpSession(session)
{
...
int CNotifyEventQueue::get_notify_fd(const UdpAddress match_addr) const
{
  int found_fd = 0;
...
int CNotifyEventQueue::get_notify_fd(const int i) const
{
...
//    m_notify_addrs = addresses;
    ...
    // allocate fd array
    m_notify_fds = new int[m_notify_addrs.size()];
    if (!m_notify_fds)
    ...
    for (int i = 0; i < m_notify_fd_count ; i++)
    {
      ...
      if ((m_notify_fds[i] = (int) socket( AF_INET, SOCK_DGRAM,0)) < 0)
      {
	  int status;
#ifdef WIN32
  ...
  for (int j = 0; j < m_notify_fd_count ; j++)
      my_holder->SnmpX11AddInput(m_notify_fds[j], m_inputId);
#endif // SNMPX11
