...
#ifndef	BBT_SOCKET_H_
#define	BBT_SOCKET_H_
#include <exception.h>
#include <address.h>
#include <iostream>
#include <string>
...
protected:
	 ...
	 * @param err string or message to pass.
	 */
	inline void error(char *err) const
	 ...
	 */
	Error sendTimeout(timeout_t to);

	/**
	 * Receive timeout.
	 */
	Error receiveTimeout(timeout_t to);
	 ...
	 */
	Error sendBuffer(unsigned bufsize);
         ...
         * @param size of buffer.
         */
        Error receiveBuffer(unsigned bufsize);

	/**
	 * Set the total buffer size bidirectionally.
	 *
	 * @return errSuccess on success
	 * @param size of buffer.
	 */
	Error bufferSize(unsigned bufsize);
...
public:
	 ...
	 * @param save sender address as peer to send reply if true.
...
public:
	...
	inline ssize_t transmit(const char *buffer, size_t len)
		{return _IORET64 ::send(so, buffer, _IOLEN64 len, MSG_DONTWAIT);}
...
public:
	 ...
	 * @param buffer size for accepted streams.
	 */
	 ...
	 * @param backlog size of connection request queue.
	 */
	TCPSocket(const char *service, unsigned backlog = 5, unsigned mss = 536);
 ...
 * @short bound server for TCP streams and sessions.
 */
class TCPV6Socket : protected Socket
{
private:
	int segsize;
	void setSegmentSize(unsigned mss);
...
public:
	 ...
	 * @param backlog size of connection request queue.
	 */
	TCPV6Socket(const char *service, unsigned backlog = 5, unsigned mss = 536);
...
protected:
	 ...
	 * @param size of stream buffer
	 */
	void connect(const char *service, unsigned mss = 536);
...
public:
	 ...
	 * @param size of streaming buffers.
	 * @param throwflag flag to throw errors.
	 * @param timeout for all operations.
	 */
	TCPStream(const char *service, Family fam = IPV4, unsigned mss = 536, bool throwflag = false, timeout_t to = 0);
 ...
 */
class TCPSession : public Thread, public TCPStream
{
private:
 ...
 **/
class SimpleTCPStream : protected Socket
{
private:
...
public:
	SockException(const std::string &str, Socket::Error socketError, 
		long systemError = 0) : IOException(str, systemError), 
