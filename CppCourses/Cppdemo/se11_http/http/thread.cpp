...
//	rc = pthread_create(&_tid, &_attr, exec_t(&bbt_exec_handler), this);
...
int Thread::start(Semaphore *st)
{
	...
	_start = st;
	return pthread_create(&priv->_tid, &priv->_attr, exec_t(&bbt_exec_handler), this);
}
...
int Thread::detach(Semaphore *st)
{
	...
	_start = st;
	rtn = pthread_create(&priv->_tid, &priv->_attr, exec_t(&bbt_exec_handler), this);
	if(!rtn && priv->_tid)
		return 0;
	return -1;
}
...
inline void ThreadImpl::ThreadExecHandler(Thread *th)
{
	...
	pthread_cleanup_push(bbt_thread_cleanup,th);
	th->initial();
